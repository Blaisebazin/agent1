...
function effectiveStatus(status) {
  const expired = status.expiresAt && Date.now() >= Date.parse(status.expiresAt);
  return { ...status, active: Boolean(status.active) && !expired };
}
...
app.get('/api/status', async (req, res) => {
  ...
  res.json(effectiveStatus(status));
});
...
app.post('/api/status', requireAdmin, async (req, res) => {
  ...
  const durationMinutes = Number.isFinite(body.durationMinutes)
    ? Math.max(0, Math.min(10080, Math.round(body.durationMinutes)))
    : 0;
  ...
  const status = {
    ...
    expiresAt: active && durationMinutes > 0
...
app.get('/api/favorites', requireAdmin, async (req, res) => {
  res.json(await readFavorites());
});

app.post('/api/favorites', requireAdmin, async (req, res) => {
  ...
  if (favorites.length >= MAX_FAVORITES) {
    return res.status(400).json({ error: `Limite de ${MAX_FAVORITES} favoris atteinte.` });
  }

  const favorite = { id: crypto.randomUUID(), name, address };
  favorites.push(favorite);
  await writeFavorites(favorites);
  res.status(201).json(favorite);
});

app.delete('/api/favorites/:id', requireAdmin, async (req, res) => {
  const favorites = await readFavorites();
  const next = favorites.filter((f) => f.id !== req.params.id);
